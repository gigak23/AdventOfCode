...
func countValidPassword(data Separations) bool {
	...
	checkValid := false
	checkLetterMin := false
	checkLetterMax := false
	fmt.Println(data.letter)
	for i := 0; i < stringLength; i++ {
		if i+1 == data.minNum && string(data.text[i]) == data.letter {
			checkLetterMin = true
		}
		if i+1 == data.maxNum && string(data.text[i]) == data.letter {
			checkLetterMax = true
		}
	}

	if checkLetterMin != checkLetterMax {
		checkValid = true
	}
